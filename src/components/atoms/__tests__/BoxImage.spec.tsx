...
describe('BoxImage', () => {
  it('renders placeholder when no poster', () => {
    const { getByText } = render(<BoxImage moviePosterPath={'' as any} size="w300" />);
    expect(getByText(/Imagem/i)).toBeTruthy();
  });

  it('renders <img> with computed src when poster exists', () => {
    const { getByAltText } = render(<BoxImage moviePosterPath="/abc.jpg" size="w300" />);
    const img = getByAltText('Poster do filme') as HTMLImageElement;
